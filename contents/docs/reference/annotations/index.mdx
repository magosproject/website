...
`ValidatingPolicy` resources targeted by Magos are selected via `Project.spec.validation.policySelector` or `Workspace.spec.validation.policySelector`. A common convention is to label policies with categories so projects can opt-in:
...
The label key is your choice; the `policy.magosproject.io/category` prefix is just a recommendation that keeps Magos-targeted policies easy to spot.
